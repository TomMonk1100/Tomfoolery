...
export function generateLevel(level: number, difficulty: Difficulty, rng: DeterministicRng): GeneratedLevel {
  const segments = 24;
  const padWidth = Math.max(104, 148 - Math.min(38, level * 1.2));
  const padX = 690 + rng.int(-100, 100);
  const points = [{ x: 0, y: WORLD_HEIGHT - 62 }];
  let y = WORLD_HEIGHT - 82;
  for (let i = 1; i <= segments; i += 1) {
    const x = (WORLD_WIDTH / segments) * i;
    const wave = Math.sin(i * 0.85 + level * 0.23) * (level < 4 ? 12 : 25);
    y = Math.max(WORLD_HEIGHT - 180, Math.min(WORLD_HEIGHT - 42, y + rng.int(-16, 16) + wave * 0.24));
    if (x > padX - padWidth && x < padX + padWidth) y = WORLD_HEIGHT - 82;
    points.push({ x, y });
  }
  ...
  for (let i = 0; i < hazardCount; i += 1) {
    const x = rng.int(170, 880); const yHazard = rng.int(110, Math.max(150, Math.floor(padY - 120)));
