...
export async function createRenderer(canvas: HTMLCanvasElement, viewport: HTMLElement, quality: QualityTier, onFailure: (message: string) => void): Promise<Renderer> {
  ...
  const resize = () => { const rect = viewport.getBoundingClientRect(); scale = Math.min(rect.width / WORLD_WIDTH, rect.height / WORLD_HEIGHT); offsetX = (rect.width - WORLD_WIDTH * scale) / 2; offsetY = (rect.height - WORLD_HEIGHT * scale) / 2; app.renderer.resize(Math.max(1, Math.floor(rect.width)), Math.max(1, Math.floor(rect.height))); root.scale.set(scale); root.position.set(offsetX, offsetY); if (background) { background.width = WORLD_WIDTH; background.height = WORLD_HEIGHT; } };
  const worldPoint = (x: number, y: number) => ({ x, y });
  const render = (state: GameState, alpha: number) => {
    const current = state.world.ship; const previous = state.previousShip; const x = previous.x + (current.x - previous.x) * alpha; const y = previous.y + (current.y - previous.y) * alpha; const angle = previous.angle + (current.angle - previous.angle) * alpha;
    terrain.clear().moveTo(0, WORLD_HEIGHT).lineTo(0, state.world.terrain.points[0].y); state.world.terrain.points.forEach((point) => terrain.lineTo(point.x, point.y)); terrain.lineTo(WORLD_WIDTH, WORLD_HEIGHT).closePath().fill({ color: 0x2d3440, alpha: 0.92 }).stroke({ color: C.slate, width: 3, alpha: 0.9 });
    ...
    ship.clear(); ship.position.set(x, y); ship.rotation = angle; ship.roundRect(-18, -15, 36, 28, 9).fill(C.cream).stroke({ color: C.copper, width: 2.5 }); ship.roundRect(-9, -10, 18, 12, 5).fill(0x6e9ca4).stroke({ color: C.night, width: 2 }); ship.moveTo(-13, 12).lineTo(-18, 24).lineTo(-7, 24).stroke({ color: C.copper, width: 4 }); ship.moveTo(13, 12).lineTo(18, 24).lineTo(7, 24).stroke({ color: C.copper, width: 4 }); ship.circle(0, -4, 2).fill(C.danger);
    if (current.thrusting) { fxLayer.clear().ellipse(x, y + 30, 9 + Math.sin(state.world.time * 42) * 2, 20).fill({ color: 0xffbe57, alpha: 0.88 }).ellipse(x, y + 33, 4, 13).fill({ color: C.signal, alpha: 0.95 }); } else fxLayer.clear();
