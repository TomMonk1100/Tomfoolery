...
export interface InputFrame { rotate: -1 | 0 | 1; thrust: boolean; abilityPressed: boolean; bothTurnHeld: boolean; kickLeftPressed: boolean; kickRightPressed: boolean; }
...
export interface ShipState { x: number; y: number; vx: number; vy: number; angle: number; angularVelocity: number; fuel: number; alive: boolean; thrusting: boolean; }
...
export interface RunState { seed: number; rulesetVersion: string; difficulty: Difficulty; level: number; upgrades: UpgradeId[]; score: number; stardust: number; stats: RunStats; rng: RngStreams; }
export interface WorldState { terrain: TerrainState; ship: ShipState; hazards: HazardState[]; time: number; wind: number; gravity: number; landing: LandingStatus; message: string; }
