...
The known playable legacy runtime is restored at `/game`. The Pixi replacement remains available at `/game-next` with an explicit development label; `/game-legacy` remains available for direct comparison. The focused recovery commit has been pushed to `main`, triggering the repository's normal deployment workflow.
